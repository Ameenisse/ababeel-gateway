...
function LandingPage() {
  ...
  const competitions = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("competitions")
        .select(
          "id, title, short_description, competition_type, competition_date, registration_close_date, banner_url, status",
        )
  ...
  const admissionStatus = (() => {
    if (!adm) return { label: "Open", variant: "success" as const };
    const now = new Date();
    ...
    if (adm.is_open === false) return { label: "Closed", variant: "destructive" as const };
      ...
      <section id="admission" className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="rounded-3xl border border-border/60 bg-card p-6 shadow-soft sm:p-10">
          <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="mt-3 flex flex-wrap gap-4 text-sm text-muted-foreground">
                  {adm?.opening_date && (
                    <span className="inline-flex items-center gap-1">
                      ...
                      {format(new Date(adm.opening_date), "PP")}
                    </span>
                  )}
                  {adm?.closing_date && (
                    <span className="inline-flex items-center gap-1">
                      <Calendar className="h-4 w-4" /> Closes{" "}
                      {format(new Date(adm.closing_date), "PP")}
                    </span>
                  )}
                </div>
              )}
            ...
            <Button
              size="lg"
              disabled={admissionStatus.variant !== "success"}
              asChild={admissionStatus.variant === "success"}
            >
      ...
      <section id="competitions" className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        ...
        {competitions.isLoading ? (
          ...
          <p className="text-muted-foreground">
            No competitions are currently open. Please check back later.
          </p>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {competitions.data!.map((c) => (
              <Card
                key={c.id}
                className="overflow-hidden border-border/60 transition-shadow hover:shadow-glow"
              >
                {c.banner_url ? (
                  <div
              ...
              <Card key={a.id} className="border-border/60">
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {a.priority === "urgent" && (
                      <StatusBadge variant="destructive">Urgent</StatusBadge>
                    )}
