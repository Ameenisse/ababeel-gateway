...
export const Route = createFileRoute("/staff/classes")({
  ssr: false,
  head: () => ({ meta: [{ title: "My Classes — Staff" }, { name: "robots", content: "noindex,nofollow" }] }),
  component: Page,
});

type ClassRow = { id: string; class_name: string; class_code: string; session: string | null; status: string };
...
function Page() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!uid) return;
      const { data: term } = await supabase.from("academic_terms").select("id").eq("status", "active").maybeSingle();
  ...
  const toggle = useCallback(async (classId: string) => {
  ...
  return (
    <RoleShell role="staff" title="My Classes">
      <div className="space-y-3">
            ...
            <CardHeader className="cursor-pointer select-none" onClick={() => toggle(c.id)}>
              <CardTitle className="flex items-center justify-between text-base">
                <span className="flex items-center gap-2">
                  ...
                  {c.class_name} <span className="text-xs text-muted-foreground">· {c.class_code}</span>
                </span>
                    ...
                    <div key={s.id} className="flex items-center justify-between rounded-md border border-border/60 p-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button asChild size="sm" variant="outline">
                          <Link to="/staff/term-report/$studentId" params={{ studentId: s.id }}>Report</Link>
                        </Button>
                      </div>
                    </div>
                  );
        ...
        {classes.length === 0 && (
          <Card><CardContent className="p-6 text-sm text-muted-foreground flex items-center gap-2"><Users className="h-4 w-4" />You are not assigned to any classes.</CardContent></Card>
        )}
      </div>
    </RoleShell>
  );
