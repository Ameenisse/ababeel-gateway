...
function Page() {
  ...
  const load = useCallback(async () => {
    ...
    for (const r of (reports ?? []) as {
      ...
      map.set(r.student_id, { id: r.id, status: r.status });
    const list: Row[] = ((students ?? []) as never[]).map((s) => {
      ...
      return {
        ...
        class_name: st.classes?.class_name ?? null,
        report_id: r?.id ?? null,
      ...
      q
        ? list.filter(
            (r) =>
              ...
              (r.student_number ?? "").toLowerCase().includes(q.toLowerCase()),
          )
        : list,
    );
  }, [termId, classId, q]);
  ...
  async function handlePublish(r: Row) {
    try {
      await publishReport({ data: { student_id: r.student_id, term_id: r.term_id } });
  ...
  async function handleUnpublish(r: Row) {
    ...
    try {
      await unpublishReport({ data: { report_id: r.report_id } });
  ...
  async function preview(r: Row) {
    try {
      if (r.status === "published" && r.report_id) {
        ...
        const s = await previewReportSnapshot({
          data: { student_id: r.student_id, term_id: r.term_id },
        });
  ...
  return (
    <RoleShell role="admin" title="Progress Reports">
      ...
      <div className="mt-4 divide-y rounded-md border bg-card">
          ...
          <div key={r.student_id} className="flex flex-col gap-2 p-3 sm:flex-row sm:items-center">
            <div className="flex-1">
              <div className="font-medium">
                {r.full_name}{" "}
                <span className="text-xs text-muted-foreground">{r.student_number}</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  navigate({
                    to: "/staff/term-report/$studentId",
              ...
              {r.status !== "published" ? (
                <Button size="sm" onClick={() => handlePublish(r)}>
        ...
        {rows.length === 0 && (
          <div className="p-6 text-sm text-muted-foreground">No students match.</div>
        )}
      </div>
