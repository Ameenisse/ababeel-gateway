...
export default defineTool({
  name: "get_student",
  title: "Get student",
  description: "Fetch a single student's full profile by their UUID. RLS applies to the signed-in user.",
  inputSchema: {
  ...
  handler: async ({ student_id }, ctx) => {
    ...
    const { data, error } = await supabase.from("students").select("*").eq("id", student_id).maybeSingle();
